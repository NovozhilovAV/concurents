...
def worker():    # функция работник
    while True:
        item = q.get()    # текущий элемент в очереди
        download_page(item)
        q.task_done()
...
def main():
    ...
    for url in urls:
        q.put(url)    # получили очередь из ссылок

    # создаем потоки и каждый поток отправляем в воркер
    for i in range(20):
        Thread(target=worker, daemon=True).start()    # создание объекта
